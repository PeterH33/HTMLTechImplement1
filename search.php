...
<body>
    ...
    if ($DBConnect == false) {
        ...
        $SQLString = "SELECT 
            ...
            WHERE 
            p.firstName = '$searchTerm' 
            OR p.lastName = '$searchTerm' 
            OR p.address LIKE '%$searchTerm%' 
            OR p.cityKey IN (SELECT cityKey FROM cities WHERE city = '$searchTerm') 
            OR p.stateKey IN (SELECT stateKey FROM states WHERE state = '$searchTerm') 
            OR p.zipCodeKey IN (SELECT zipCodeKey FROM zipCodes WHERE zipCode = '$searchTerm') 
            OR p.colorKey IN (SELECT colorKey FROM colors WHERE color = '$searchTerm') 
            OR p.favNumber = '$searchTerm' 
            OR p.dayKey IN (SELECT dayKey FROM days WHERE day = '$searchTerm') 
            OR p.ts LIKE '%$searchTerm%'
        ";
